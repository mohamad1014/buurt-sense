...
| Inference | Local video action recognition (lightweight) | Implemented (client-side TFLite) | Browser samples 16 frames, center-crops to 112×112, normalises with Kinetics-400 stats, and feeds the Qualcomm ResNet-Mixed-Convolution TFLite model inside a dedicated Web Worker so the UI thread stays responsive. The lighter **w8a16** asset (`/static/tflite/ResNet-Mixed-Convolution_w8a16.tflite`) can be opted into with `window.BUURT_ENABLE_W8A16 = true` (or `localStorage.setItem('buurt-enable-w8a16','1')`), but because the web runtime currently lacks the needed dynamic-linking support we default to the upstream float weights after detecting an unsupported browser. |
